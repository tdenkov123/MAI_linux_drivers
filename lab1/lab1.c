#include <linux/cdev.h>
#include <linux/device.h>
#include <linux/fs.h>
#include <linux/init.h>
#include <linux/module.h>
#include <linux/printk.h>
#include <linux/wait.h>
#include <linux/jiffies.h>
#include "ioctl_macro.h"

#define DRIVER_NAME "lab1"
#define DEVICE_NAME "lab1_dev"
#define CLASS_NAME "lab1_class"
#define HISTOGRAM_MAX_LEN 500

static int buf;
static bool buf_is_empty = true;
static size_t histogram_len = 0;
static size_t histogram_buf[HISTOGRAM_MAX_LEN];
static ulong time_from_bin_start = 0;
static ulong last_write_time;

static dev_t dev_no;
static struct cdev dev;
static struct class *dev_class;
static struct device *device;

static int device_open(struct inode *inode, struct file *file);
static int device_release(struct inode *inode, struct file *file);
static ssize_t device_read(struct file *flip, char __user *user_buf, size_t count, loff_t *offset);
static ssize_t device_write(struct file *flip, const char __user *user_buf, size_t count, loff_t *offset);
static long device_ioctl(struct file *flip, unsigned int cmd, unsigned long arg);

const struct file_operations fops = {
	.open = device_open,
	.release = device_release,
	.read = device_read,
	.write = device_write,
	.unlocked_ioctl = device_ioctl,
	.owner = THIS_MODULE,
};

static int device_open(struct inode *inode, struct file *file) {
	printk(KERN_INFO DRIVER_NAME ": device_open: trying to open the device\n");
	return 0;
}

static int device_release(struct inode *inode, struct file *file) {
	printk(KERN_INFO DRIVER_NAME ": device_release: trying to release the device\n");
	return 0;
}

static ssize_t device_read(struct file *flip, char __user *user_buf, size_t count, loff_t *offset) {
	printk(KERN_INFO DRIVER_NAME
		": device_read: count=%zu, buf=%zu, buf_is_empty=%s\n",
		count, buf, buf_is_empty ? "true" : "false");

	if (count != sizeof(buf)) {
		printk(KERN_INFO DRIVER_NAME ": device_read: `count` must %lld\n",
			sizeof(buf));
		return -EINVAL;
	}

	if (buf_is_empty) {
		printk(KERN_INFO DRIVER_NAME ": device_read: End Of File\n");
		return 0;
	}

	if (copy_to_user(user_buf, &buf, sizeof(buf))) {
		printk(KERN_ERR DRIVER_NAME ": device_read: copy_to_user failed\n");
		return -EFAULT;
	}

	printk(KERN_INFO DRIVER_NAME ": device_read: successfully read %lu bytes\n",
		sizeof(buf));

	histogram_buf[histogram_len] += 1;
	const ulong time_between_write_and_read = jiffies - last_write_time;
	time_from_bin_start += time_between_write_and_read;
	if (jiffies_to_usecs(time_from_bin_start) >= 50) {
		time_from_bin_start = 0;
		histogram_len += 1;
	}

	return sizeof(buf);
}

static ssize_t device_write(struct file *flip, const char __user *user_buf, size_t count, loff_t *offset) {
	printk(KERN_INFO DRIVER_NAME ": device_write: count=%zu, buf=%zu\n", count, buf);

	if (count != sizeof(buf)) {
		printk(KERN_INFO DRIVER_NAME ": device_write: `count` must %lld\n",
			sizeof(buf));
		return -EINVAL;
	}

	if (copy_from_user(&buf, user_buf, count)) {
		printk(KERN_ERR DRIVER_NAME ": device_write: copy_from_user failed\n");
		return -EFAULT;
	}

	buf_is_empty = false;

	printk(KERN_INFO DRIVER_NAME ": device_write: successfully wrote %lu bytes\n",
		count);

	last_write_time = jiffies;

	return count;
}

static long device_ioctl(struct file *flip, unsigned int cmd, unsigned long arg) {
	printk(KERN_INFO DRIVER_NAME ": device_ioctl: cmd=0x%x\n", cmd);

	switch (cmd) {
	case DRIVER_GET_HISTOGRAM_LEN:
		printk(KERN_INFO DRIVER_NAME
			": device_ioctl: DRIVER_GET_HISTOGRAM_LEN\n");
		if (copy_to_user((typeof(histogram_len) *)arg, &histogram_len,
				 sizeof(histogram_len))) {
			printk(KERN_ERR DRIVER_NAME
			       ": device_ioctl: copy_to_user failed\n");
			return -EFAULT;
		}
		break;
	case DRIVER_GET_HISTOGRAM_BUF:
		printk(KERN_INFO DRIVER_NAME
			": device_ioctl: DRIVER_GET_HISTOGRAM_BUF\n");
		ulong err;
		if ((err = copy_to_user((size_t *)arg, histogram_buf,
				 histogram_len * sizeof(size_t)))) {
			printk(KERN_ERR DRIVER_NAME
			       ": device_ioctl: copy_to_user failed: %ld\n", err);
			return -EFAULT;
		}
		break;
	default:
		printk(KERN_ERR DRIVER_NAME ": device_ioctl: Unknown command: %d\n",
		       cmd);
		break;
	}

	return 0;
}

static int __init init(void) {
	printk(KERN_INFO DRIVER_NAME ": trying to initialize the driver\n");

	int res = 0;

	res = alloc_chrdev_region(&dev_no, 0, 1, DEVICE_NAME);
	if (res < 0) {
		printk(KERN_ERR DRIVER_NAME
		       ": failed to allocate device numbers with error %d\n",
		       res);
		return res;
	}

	cdev_init(&dev, &fops);
	dev.owner = THIS_MODULE;

	res = cdev_add(&dev, dev_no, 1);
	if (res < 0) {
		printk(KERN_ERR DRIVER_NAME
		       ": failed to add character device with error %d\n",
		       res);
		unregister_chrdev_region(dev_no, 1);
		return res;
	}

	dev_class = class_create(CLASS_NAME);
	if (IS_ERR(dev_class)) {
		printk(KERN_ERR DRIVER_NAME ": class_create failed\n");
		res = PTR_ERR(dev_class);
		cdev_del(&dev);
		unregister_chrdev_region(dev_no, 1);
		return res;
	}

	device = device_create(dev_class, NULL, dev_no, NULL, DEVICE_NAME);
	if (IS_ERR(device)) {
		printk(KERN_ERR DRIVER_NAME ": failed to create device node\n");
		res = PTR_ERR(device);
		class_destroy(dev_class);
		cdev_del(&dev);
		unregister_chrdev_region(dev_no, 1);
		return res;
	}

	printk(KERN_INFO DRIVER_NAME ": successfully initialized the driver\n");
	return 0;
}

static void __exit cleanup(void) {
	printk(KERN_INFO DRIVER_NAME ": cleaning up driver\n");

	device_destroy(dev_class, dev_no);
	class_destroy(dev_class);
	cdev_del(&dev);
	unregister_chrdev_region(dev_no, 1);

	printk(KERN_INFO DRIVER_NAME ": driver unloaded successfully\n");
}

module_init(init);
module_exit(cleanup);

MODULE_DESCRIPTION("MAI drivers training");
MODULE_LICENSE("GPL");

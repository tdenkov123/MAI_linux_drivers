...
static ssize_t io_address_show(struct device *dev, struct device_attribute *attr, char *buf);
...
static struct device_attribute io_address_attr = {
    .attr = {
        .name = "io_address",
        .mode = S_IWUSR | S_IRUGO,
    },
    .show = io_address_show,
};
...
static int pci_driver_probe(struct pci_dev *dev, const struct pci_device_id *ent) {
	...
	if (!fake_netdev) {
		printk(KERN_ERR DRIVER_NAME
		       ": pci_driver_probe: Failed to alloc_etherdev\n");
		return -ENOMEM;
	}
	...
	pci_res_start = pci_resource_start(dev, 0);
	pci_res_len = pci_resource_len(dev, 0);
	u8 *mapped_ptr = ioremap(pci_res_start, pci_res_len);

	int j = 0;
	const unsigned char *const ptr = mapped_ptr;
	for (int i = 0; i < pci_res_len; i++) {
		if (ptr[i] == 0x70) {
	...
	result = device_create_file(&dev->dev, &io_address_attr);
	if (result != 0) goto err_free_netdev;
...
static void pci_driver_remove(struct pci_dev *dev) {
	...
	if (fake_netdev) {
		...
		free_netdev(fake_netdev);
		fake_netdev = NULL;
		device_remove_file(&dev->dev, &io_address_attr);
	}
}
...
{
	printk(KERN_INFO DRIVER_NAME ": io_address_show\n");

	u8 *const mapped_ptr = ioremap(pci_res_start, pci_res_len);

	const int written_bytes_count =
		sprintf(buf,
			"Pci resource start address: %lx\n"
			"Pci resource length: %d\n"
			"Mapped address: %p\n",
			pci_res_start, pci_res_len, mapped_ptr);
